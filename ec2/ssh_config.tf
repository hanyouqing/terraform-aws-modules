...
${join("\n", [for cidr in try(local.vpc_remote.public_subnet_cidrs, []) : "Host ${replace(cidr, "/^(\\d+\\.\\d+\\.\\d+)\\..*$/", "$1.*")}\n    User ubuntu\n    ProxyJump ${var.name_prefix}-${var.environment}\n    IdentityFile ~/.ssh/${var.name_prefix}-${var.environment}"])}

# Access Private Subnet Instances (via EC2 Instance) - ec2-user
${join("\n", [for cidr in try(local.vpc_remote.private_subnet_cidrs, []) : "Host ${replace(cidr, "/^(\\d+\\.\\d+\\.\\d+)\\..*$/", "$1.*")}\n    User ec2-user\n    ProxyJump ${var.name_prefix}-${var.environment}\n    IdentityFile ~/.ssh/${var.name_prefix}-${var.environment}"])}
EOT
